...
class P4GChatSystemPlanner(P4GSystemPlanner):
    ...
    def predict(self, state:DialogSession) -> "Tuple[np.ndarray, float]":
        ...
        logger.info(
            "gen_model = %s.%s",
            self.generation_model.__class__.__module__,
            self.generation_model.__class__.__name__,
        )
...
class PersuaderChatModel(PersuaderModel):
    ...
    def get_utterance_batched(self, state:DialogSession, action:int, batch:int=3) -> List[str]:
        ...
        if len(state) == 0:
            messages.append({
                'role': 'user',
                'content': f'{PersuasionGame.USR}: Hello.\n{da_prompt}'
            })
        else:
            assert(state[-1][0] == PersuasionGame.USR)
            messages += self.__proccess_chat_exp(state, max_hist_num_turns=self.max_hist_num_turns)

        # 强制 Persuader 只输出一句简短回复（与其他模型 prompt 对齐）
        messages.append({
            'role': 'system',
            'content': f'{da_prompt} Please reply with only one short and succinct sentence.'
        })
        gen_args = {
            **self.inference_args,
        ...
        # 如果返回空列表（理论上不应该发生，因为会抛出异常触发重试），返回默认值
        if len(sys_resps) == 0:
            logger.error(
                f"get_utterance_batched returned empty list after retries. "
                f"DA: {da}, batch: {batch}. "
                f"Returning default placeholder."
            )
            # 返回一个默认的占位符，避免 IndexError
            return [f"[Content filtered for {da}]"] * batch
